...
export const CountdownHero: React.FC<CountdownHeroProps> = ({ iftarTime, fajrTime, locationName, hijriDate }) => {
    ...
    return (
        <div className="relative w-full flex flex-col items-center justify-center py-1 sm:py-6 md:py-10">
            ...
            <h2 className="text-[14px] sm:text-3xl md:text-5xl lg:text-5xl xl:text-5xl 2xl:text-6xl font-bold text-white mb-1 sm:mb-6 md:mb-4 mt-0 sm:mt-4 md:mt-4 tracking-tight text-center transition-all duration-700">
                Time until <span className="text-emerald-400">Iftar</span>
            </h2>

            <div className="flex items-start gap-1.5 sm:gap-6 md:gap-8 lg:gap-10 xl:gap-10 2xl:gap-12 transition-all duration-500">
            ...
            <div className="w-full max-w-[280px] sm:max-w-md md:max-w-xl xl:max-w-2xl 2xl:max-w-3xl mt-2 sm:mt-8 md:mt-4 px-4 transition-all duration-500">
...
const TimeUnit = ({ value, label }: { value: number; label: string }) => (
    <div className="flex flex-col items-center">
        <div className="w-14 h-20 sm:w-28 sm:h-36 md:w-36 md:h-48 lg:w-44 lg:h-56 xl:w-48 xl:h-56 2xl:w-52 2xl:h-60 bg-slate-800/50 backdrop-blur-xl border border-white/5 rounded-xl sm:rounded-[32px] 2xl:rounded-[32px] flex items-center justify-center shadow-2xl relative overflow-hidden group transition-all duration-500">
            <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
            <span className="text-xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-8xl 2xl:text-[8rem] font-bold text-emerald-400 tabular-nums tracking-tighter glow-text">
                {value.toString().padStart(2, '0')}
            </span>
        </div>
        <span className="mt-1 sm:mt-3 xl:mt-3 text-[7px] sm:text-xs md:text-sm xl:text-sm 2xl:text-base font-semibold text-slate-500 tracking-widest">{label}</span>
    </div>
);
